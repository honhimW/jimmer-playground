...
class IOperator {
    ...
    IOperator(SaveContext ctx, boolean ownerAcceptanceRequired) {
        this.ctx = ctx;
    ...
    private void insert(
        ...
        for (PropertyGetter getter : IShape.fullOf(sqlClient, tableType).getGetters()) {
        ...
        if (sequenceIdGenerator != null) {
            ...
            Shape fullShape = IShape.fullOf(sqlClient, tableType);
            builder.separator();
    ...
    private int[] update(
        ...
        JSqlClientImplementor sqlClient = ctx.options.getSqlClient();
        List<PropertyGetter> idGetters = IShape.fullOf(sqlClient, shape.getType()).getIdGetters();
        ...
        if (returning != null) {
            ...
            Dialect.UpdateContext updateContext = new UpdateContextImpl(
                    ...
                    tableType,
                    shape,
                    IShape.fullOf(sqlClient, shape.getType()).getIdGetters().get(0),
                    keyProps,
                    assignments,
                    discriminatorProp,
                    discriminatorGuardProp,
    ...
    private void updateJoinedChildWithRootGuard(
        ...
        MetadataStrategy strategy = sqlClient.getMetadataStrategy();
        List<PropertyGetter> idGetters = IShape.fullOf(sqlClient, tableType).getIdGetters();
    ...
    private class UpsertContextImpl implements Dialect.UpsertContext {
        ...
        UpsertContextImpl(
            ...
            this.generatedIdGetter = generatedIdProp != null ?
                    IShape.fullOf(builder.sqlClient(), tableType)
                            .getIdGetters()
                            .get(0) :
                    null;
            this.fakeUpdateGetter = fakeUpdateGetter(ctx.options.getSqlClient(), tableType);
