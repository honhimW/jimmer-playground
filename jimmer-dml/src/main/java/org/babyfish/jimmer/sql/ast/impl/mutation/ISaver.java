...
public class ISaver {
    ...
    ISaver(SaveContext ctx) {
        this.ctx = ctx;
    }

    @SuppressWarnings("unchecked")
    public <E> SimpleSaveResult<E> save(E entity) {
        ImmutableType immutableType = getType(entity.getClass());
    ...
    private void saveAllJoinedInsertImpl(List<DraftSpi> drafts) {
        ...
        List<ISaveOperation> operations = new ArrayList<>(groupMap.size());
        for (Map.Entry<ImmutableType, List<DraftSpi>> e : groupMap.entrySet()) {
            ...
            org.babyfish.jimmer.sql.ast.impl.mutation.ISaver groupSaver = new org.babyfish.jimmer.sql.ast.impl.mutation.ISaver(groupCtx);
        ...
        for (int i = 0; i < operations.size(); i++) {
            ISaveOperation operation = operations.get(i);
    ...
    private void saveAllImpl(List<DraftSpi> drafts) {
        ...
        ISaveOperation operation = prepareSave(drafts);
        SaveSelfResult selfResult = saveSelf(operation);
        finishSave(operation, selfResult);
    }

    ISaveOperation prepareSave(List<DraftSpi> drafts) {
        ...
        return new ISaveOperation(this, drafts, preHandler, isOwnerAcceptanceRequired(preHandler));
    }

    private void finishSave(ISaveOperation operation, SaveSelfResult selfResult) {
    ...
    void finishAssociations(ISaveOperation operation, SaveSelfResult selfResult) {
    ...
    private SaveSelfResult saveSelf(ISaveOperation operation) {
    ...
    private List<SaveSelfResult> saveJoinedInsertSelf(
        ...
        List<ISaveOperation> operations
    ) {
        ...
        for (ISaveOperation operation : operations) {
    ...
    private void insertJoinedRootStages(
        ...
        for (DraftSpi draft : drafts) {
            ...
            Shape shape = IOperator.joinedRootShape(sqlClient, rootType, draft);
    ...
    private void insertJoinedChildStages(
        ...
        for (DraftSpi draft : drafts) {
            ...
            for (ImmutableType tableType : IOperator.joinedTableTypes(rootType, draft.__type())) {
                Shape shape = IOperator.joinedStageShape(sqlClient, previousTableType, tableType, draft);
        ...
        for (StageGroup group : groupMap.values()) {
            ...
            new IOperator(stageCtx).insertJoinedStage(
                SaveBatches.of(group.shape, group.entities, SaveMode.INSERT_ONLY, SaveMode.INSERT_ONLY),
                group.tableType
            );
        }
    ...
    private SaveSelfResult saveSelf(PreHandler preHandler, boolean ownerAcceptanceRequired) {
        IOperator operator = new IOperator(ctx, ownerAcceptanceRequired);
        boolean detach = false;
        ...
        for (Batch<DraftSpi> batch : preHandler.batches()) {
            ...
            IOperator.MutationRows mutationRows = saveBatch(preHandler, operator, batch);
    ...
    private IOperator.MutationRows saveBatch(PreHandler preHandler, IOperator operator, Batch<DraftSpi> batch) {
        switch (batch.mode()) {
            case INSERT_ONLY:
